  ...
  implements OnInit, AfterViewInit, OnDestroy, OnChanges
{
  ...
  @Input() caseSensitiveSearch: boolean;
        ...
        if (!this.lazyLoad) {
          ...
          if (this.isStringArray) {
            filteredSource = this.source.filter((x) =>
              regexp.test(x?.toString())
            );
          } else {
            filteredSource = this.source.filter((x) =>
              this._getRecursiveValues(x).some((value) =>
                regexp.test(`${value ?? ''}`)
              )
            );
          }
  ...
  // simple recursive algorithms to get all values in an object as an array
  private _getRecursiveValues(object: any) {
    const values = [];
    if (typeof object == 'object') {
      // dealing with a nested object
      for (const key of Object.keys(object)) {
        values.push(...this._getRecursiveValues(object[key]));
      }
